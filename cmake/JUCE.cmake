...
function( addJUCE_standalone name_ sources_)

    set( CURRENT_TARGET "${name_}" )

    if(WIN32)
      add_executable ( ${CURRENT_TARGET} WIN32 ${sources_} )
    else()
      add_executable ( ${CURRENT_TARGET} ${sources_} )
    endif()

    addJUCE( ${CURRENT_TARGET} )

    target_compile_definitions( ${CURRENT_TARGET} PRIVATE JucePlugin_Build_Standalone=1 )

    set(juce_library_audio_plugin_client_standalone
    ${JUCE_LIBRARY_CODE_DIR}/juce_audio_plugin_client_Standalone.cpp
    )

    target_sources( ${CURRENT_TARGET} PRIVATE ${juce_library_audio_plugin_client_standalone} )
    
    if(LINUX)
      target_link_libraries(${CURRENT_TARGET} PRIVATE -ldl -pthread)
    endif(LINUX)
...
function( addJUCEPlugins name_ sources_ )
  ...
  addJUCE_standalone( ${name_} "${sources_}" )
